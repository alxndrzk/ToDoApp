...
class TaskRepository(private val tasksDao: TaskDao) {
    ...
    fun getTasks(filter: TasksFilterType): LiveData<PagedList<Task>> {
        val pegeListConfig = PagedList.Config.Builder()
            .setPageSize(PAGE_SIZE)
            .setEnablePlaceholders(PLACEHOLDERS)
            .build()

        val filterQuery = FilterUtils.getFilteredQuery(filter)
        val getFilterQuery = tasksDao.getTasks(filterQuery)

        return LivePagedListBuilder(getFilterQuery, pegeListConfig).build()
    }
